...
export function hashWithSalt(password: string): string {
    ...
    const hashedPassword: string = scryptSync(password, salt, 64).toString('base64');

    return `${salt}:${hashedPassword}`;
}

export function compareHash(originalPassword: string, strangePassword: string): boolean {
    return this.hashPassword(originalPassword) === this.hashPassword(strangePassword);
}

export function compareHashWithSalt(originalPassword: string, strangePassword: string): boolean {
